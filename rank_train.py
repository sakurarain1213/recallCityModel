...
def _vectorized_neg_sample(base: pd.DataFrame, random_seed: int) -> pd.DataFrame:
    """
    v8 动态负采样策略：
    1. 保留所有正样本（Relevance > 0）
    2. 保留所有静态困难负样本（HARD_NEG_CITIES_SET 中的负样本城市）
    3. 保留所有动态困难负样本（与出发城市同省份的负样本，城市代码前两位相同）
    4. 从剩余负样本中随机采样 N_RAND_NEG 个，确保模型见到全局视野
    ...
    # 2. 静态困难负样本：一二线城市（热门迁移目标）
    is_static_hard = np.isin(to_city, list(HARD_NEG_CITIES_SET))
    keep[is_neg & is_static_hard] = True

    # 3. 动态困难负样本：同省份城市（省内竞争）
    # 中国城市代码规则：前两位为省份代码（如 1301 石家庄，13 为河北省）
    # 注意：排除已经在静态困难负样本中的城市，避免重复
    to_province = to_city // 100
    from_province = from_city // 100
    is_same_province = (to_province == from_province)
    is_dynamic_hard = is_neg & is_same_province & (~is_static_hard)
    keep[is_dynamic_hard] = True

    # 4. 随机负样本：从剩余负样本中固定采样 N_RAND_NEG 个
    # remaining_neg 自动排除了：正样本 + 静态困难负样本 + 动态困难负样本
    ...
    if len(rand_idx) > 0:
        ...
        rand_df = pd.DataFrame({
            'qid': qids_rand,
            'rank': rng.random(len(rand_idx)),
            'orig_idx': rand_idx
        })
        # 按 qid 分组，每组随机排序后取前 N_RAND_NEG 个
        rand_df['grp_rank'] = rand_df.groupby('qid')['rank'].rank(method='first')
        selected_rand_idx = rand_df[rand_df['grp_rank'] <= N_RAND_NEG]['orig_idx'].values
        keep[selected_rand_idx] = True
