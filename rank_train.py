...
def main():
    ...
    train_ds = lgb.Dataset(str(BIN_DIR / "train_v11_clean.bin"))

    print("正在从 /data2 极速加载 Val 数据...")
    # 验证集必须设置 reference=train_ds，保证特征分桶边界一致
    val_ds = lgb.Dataset(str(BIN_DIR / "val_v11_clean.bin"), reference=train_ds)
    ...
    final_model_path = OUTPUT_DIR / 'model_v11_clean.txt'
