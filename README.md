...
调整 LightGBM 参数 (针对 Top K 优化)
既然只关心 Top 10 准不准，。但在二分类框架下（不要用LambdaRank），可以通过加大 Top 样本权重来实现。
修改文件: fast_train.py (在 train_batch_mode 循环内)
        # 加载 -> 打乱
        df_train = load_data_batch(batch_years, shuffle=True)
        # ... 特征对齐代码 ...
        # 🔥 激进的权重策略
        # Top 10 (Label=1.0): 权重 20
        # Rank 11-20 (Label=0.1): 权重 5 (让模型重视这些"差点选对"的错题)
        # 其他 (Label=0.0): 权重 1
        weights = df_train['Label'].apply(lambda x: 20.0 if x > 0.9 else (5.0 if x > 0.05 else 1.0))
        train_ds = lgb.Dataset(
            df_train[feats], 
            label=df_train['Label'], 
            weight=weights,  # <--- 确保传入 weights
            # ...
        )
...
【okk】特征工程优化 (User Req 4): 增加 is_same_province (是否同省) 特征。这是一个强特征。

【okk】模型参数调整 (User Req 3): 增加树的深度，因为人口流动是非线性的复杂关系。
