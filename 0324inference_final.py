"""
LambdaRank 本地敏捷推理脚本 - Windows 测试版
新增功能:
1. 灵活指定起止年份
2. 设定 Query 采样率 (sample-ratio)
3. 设定 候选城市 采样率 (city-ratio)
4. 运行前打印模型特征与结构信息
5. 运行后打印全局总体 Recall@20 指标

用法示例:
python 0324inference_final.py --start-year 2000 --end-year 2000
自动配置：

1 进程 × 28 线程（你的 CPU 核心数）
LightGBM 多线程火力全开
多年份并行推理
...
if os.name == 'nt':
    ...
    MODEL_PATH = PROJECT_ROOT / "output" / "models" / "0324ltr_model_rapid.txt"
...
def ensure_binary_model(model_path: Path) -> Path:
    ...
    if not bin_path.exists():
        print(f"[Convert] 转换为二进制格式: {bin_path}")
...
def print_model_info(model_path: Path):
    """打印树模型结构和参数特征"""
    print(f"\n{'='*60}")
    print(f" 📊 模型信息预检: {model_path.name}")
    print(f"{'='*60}")
