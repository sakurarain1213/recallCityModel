...
def train_rapid_global(train_years, train_n, val_n, train_neg, val_neg, num_rounds=2000, save_every=100):
    ...
    params = {
        ...
        # 🚀 提速核心
        'learning_rate': 0.1,           # 加大步长，加速收敛 (原0.05)
        'num_leaves': 63,               # 降低复杂度 (原127)
        'max_depth': 7,                 # 配合leaves限制深度 (原8)
        'min_child_samples': 50,        # 大数据量可以提高 (原5)

        # 🛡️ 抽样加速 + 抗过拟合
        'bagging_fraction': 0.8,        # 行抽样 80%
        'bagging_freq': 1,              # 每棵树都抽样
        'feature_fraction': 0.8,        # 列抽样 80%
    ...
    # 添加 checkpoint 回调
    if save_every > 0:
        ckpt_cb = CheckpointCallback(MODEL_DIR / "0322ltr_checkpoint", freq=save_every)
